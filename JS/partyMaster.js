...
let currentPage = 0;
let pageSize = 10;
let filteredData = [...partyMasterData];
let sortColumn = null;
...
function renderTable() {
    ...
    tbody.innerHTML = pageData.map(partyMaster => `
        <tr class="fade-in hover:bg-gray-50">
            <td class="px-6 py-4">
                <div class="flex items-center">
                    <input class="row-checkbox w-5 h-5 text-primary-500 border-gray-300 rounded focus:ring-primary-500" type="checkbox" value="${partyMaster.id}">
                </div>
            </td>
            <td class="px-6 py-4 text-gray-900 text-sm">${partyMaster.id}</td>
            <td class="px-6 py-4 text-gray-900 text-sm">${partyMaster.name}</td>
            <td class="px-6 py-4">
                <div class="flex items-center">
                    <div class="flex flex-col">
                        <a href="#!" class="text-gray-900 font-semibold hover:text-primary-500 text-sm">${partyMaster.address}</a>
                    </div>
                </div>
            </td>
            <td class="px-6 py-4 text-gray-900 text-sm">${partyMaster.gstin}</td>
            <td class="px-6 py-4 text-gray-900 text-sm">${partyMaster.contactNo}</td>
            <td class="px-6 py-4 text-gray-900 text-sm">${partyMaster.email}</td>
            <td class="px-6 py-4">
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${partyMaster.isVendor === 'Yes' ? 'badge-success' : 'badge-danger'}">
                    ${partyMaster.isVendor}
                </span>
            </td>
            <td class="px-6 py-4">
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${partyMaster.isCustomer === 'Yes' ? 'badge-success' : 'badge-danger'}">
                    ${partyMaster.isCustomer}
                </span>
            </td>
            <td class="px-6 py-4">
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${partyMaster.status === 'Active' ? 'badge-success' : 'badge-danger'}">
                    ${partyMaster.status}
                </span>
            </td>
            <td class="px-6 py-4">
...
function initializeSearch() {
    ...
    searchInput.addEventListener('input', function() {
        ...
        filteredData = partyMasterData.filter(product => 
