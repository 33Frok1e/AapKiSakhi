...
const pbillData = [
    {
        provisionalBill: "PBILL00014",
        date: "13-04-2024",
        createdBy: "Rati Krishna",
        isProcessed: "Yes",
        invoiceNumber: "INV00415",
        invoiceDte: "13-04-2024",
        conversionBy: "Rati Krishna Moharana",
    },
    {
        provisionalBill: "PBILL00014",
        date: "13-04-2024",
        createdBy: "Elina Karimova",
        isProcessed: "No",
        invoiceNumber: "INV00416",
        invoiceDte: "13-04-2024",
        conversionBy: "Rati Krishna Moharana",
    },
];
...
function renderTable() {
    ...
    tbody.innerHTML = pageData.map(pbill => `
        <tr class="fade-in hover:bg-gray-50">
            <td class="px-6 py-4">
                <div class="flex items-center">
                    <input class="row-checkbox w-5 h-5 text-primary-500 border-gray-300 rounded focus:ring-primary-500" type="checkbox" value="${pbill.id}">
                </div>
            </td>
            <td class="px-6 py-4 text-gray-900 text-sm">${pbill.provisionalBill}</td>
            <td class="px-6 py-4">
                <div class="flex items-center">
                    <div class="flex flex-col">
                        <a href="#!" class="text-gray-900 font-semibold hover:text-primary-500 text-sm">${pbill.date}</a>
                    </div>
                </div>
            </td>
            <td class="px-6 py-4 text-gray-900 text-sm">${pbill.createdBy}</td>
            <td class="px-6 py-4">
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${pbill.isProcessed === 'Yes' ? 'badge-success' : 'badge-danger'}">
                    ${pbill.isProcessed}
                </span>
            </td>
            <td class="px-6 py-4 text-gray-900 text-sm">${pbill.invoiceNumber}</td>
            <td class="px-6 py-4 text-gray-900 text-sm">${pbill.invoiceDte}</td>
            <td class="px-6 py-4 text-gray-900 text-sm">${pbill.conversionBy}</td>
            <td class="px-6 py-4">
...
function initializeSearch() {
    ...
    searchInput.addEventListener('input', function() {
        ...
        filteredData = pbillData.filter(product => 
