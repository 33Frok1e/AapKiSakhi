...
let currentPage = 0;
let pageSize = 10;
let filteredData = [...receiveOrderData];
let sortColumn = null;
        ...
        <tr class="fade-in hover:bg-gray-50">
            <td class="px-6 py-4">
                <div class="flex items-center">
                    <input class="row-checkbox w-5 h-5 text-primary-500 border-gray-300 rounded focus:ring-primary-500" type="checkbox" value="${order.id}">
                </div>
            </td>
            <td class="px-6 py-4 text-gray-900 text-sm">${order.documentNo}</td>
            <td class="px-6 py-4">
                <div class="flex items-center">
                    <div class="flex flex-col">
                        <a href="#!" class="text-gray-900 font-semibold hover:text-primary-500 text-sm">${order.entryDate}</a>
                    </div>
                </div>
            </td>
            <td class="px-6 py-4 text-gray-900 text-sm">${order.customer}</td>
            <td class="px-6 py-4 text-gray-900 text-sm">${order.amountReceived}</td>
            <td class="px-6 py-4 text-gray-900 text-sm">${order.deliveryDate}</td>
            <td class="px-6 py-4 text-gray-900 text-sm">${order.itemCount}</td>
            <td class="px-6 py-4">
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${order.status === 'Delivered' ? 'badge-success' : 'badge-danger'}">
                    ${order.status}
                </span>
            </td>
            <td class="px-6 py-4">
...
function initializeSearch() {
    ...
    searchInput.addEventListener('input', function() {
        ...
        filteredData = receiveOrderData.filter(product => 
